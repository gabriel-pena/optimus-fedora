...
el = gettext.translation('optimus-fedora', localedir='locales')
el.install()
_ = el.gettext
...
def switch_gpu(z):
  ...
  if(gpu_mode == 'hybrid'):
    ...
    if(response == gtk.ResponseType.YES):
      quit_app = True
      subprocess.call("pkexec /usr/bin/optimus-fedora nvidia", shell=True)
  elif(gpu_mode == 'nvidia'):
    gpu_sure_text = _('Are you sure you want to change to a ') + _('Hybrid Graphics') + "?"
    response = ChangeGpuDialog()
    if(response == gtk.ResponseType.YES):
      quit_app = True
      subprocess.call("pkexec /usr/bin/optimus-fedora offload", shell=True)
  if quit_app:
